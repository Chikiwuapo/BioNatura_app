...
public class ActualizarContrasena extends AppCompatActivity {
    ...
    private void enviarPassword() {
        ...
        Call<ResponseBody> call = apiService.actualizarPassword("Bearer " + token, new PasswordUpdateRequest(actual, nueva));

        call.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                if (response.isSuccessful()) {
                    Toast.makeText(ActualizarContrasena.this, "Contraseña actualizada correctamente", Toast.LENGTH_SHORT).show();
                    finish();
                } else {
                    Toast.makeText(ActualizarContrasena.this, "Error al actualizar (verifica tu contraseña actual)", Toast.LENGTH_LONG).show();
                }
            }
            @Override
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                Toast.makeText(ActualizarContrasena.this, "Error de red: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void mostrarAdvertenciaSalida() {
        boolean hayCambios = !campo1.getText().toString().isEmpty() ||
        ...
        view.findViewById(R.id.btnCancelarDialog).setOnClickListener(v -> {
            dialog.dismiss(); // Solo cerramos el mensaje y nos quedamos en la vista
        });
